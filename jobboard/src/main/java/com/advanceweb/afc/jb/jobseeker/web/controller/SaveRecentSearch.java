...
@Controller
@RequestMapping(value = "/savedRecentSearches")
public class SaveRecentSearch {
			...
			if (StringUtils.isEmpty(searchName)) {
				...
				if (isSrchNameExist) {
					...
					if (!keyWords.isEmpty()) {
						...
						recMap.put(SearchParamDTO.KEYWORDS,
								sessionMap.get(SearchParamDTO.KEYWORDS).trim());
					}
					if (!city.isEmpty()) {
						recMap.put(SearchParamDTO.CITY_STATE, city);
					}
					if (!radius.equalsIgnoreCase(MMJBCommonConstants.ZERO)) {
						recMap.put(SearchParamDTO.RADIUS, radius);
					}
