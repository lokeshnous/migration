...
public class ManageJobSeekerController {
	...
	/**
	 * Method is called to get All Folders associated to the Logged-in User	 
	 *  
	 * @param request
	 * @param response
	 * @param session
	...
	public @ResponseBody
	JSONObject getAllFolders(HttpServletRequest request,
			HttpServletResponse response, HttpSession session,
			ManageJobSeekerForm manageJobSeekerForm,  @RequestParam("folderId") int folderId) {
		JSONObject foldersJSONObject = new JSONObject();
		if (folderId == 0) {
			List<AdmFolderDTO> admFolderDTOList = new ArrayList<AdmFolderDTO>();
			try {
				admFolderDTOList = manageJobSeekerService
					.folderDetailList((Integer) session
							.getAttribute(MMJBCommonConstants.USER_ID));
					foldersJSONObject = convertFolderDtoToJsonObject(admFolderDTOList); 
			} catch (JobBoardServiceException jbex) {
				LOGGER.error("Error occured while Retriving folder json_data", jbex);
			}
		}
	...
	/**
	 * Convert the Folder DTO to the required JSON format to display as Tree Structure
	 * @param admFolderDTOList
	 * @return
	 */
	private JSONObject convertFolderDtoToJsonObject(
			List<AdmFolderDTO> admFolderDTOList) {
		
		//Initial Folder with Name "My Folders"
		JSONObject folderObject = new JSONObject();
		folderObject.put("data", "My Folders");
		//Folder Open/Close Status
		folderObject.put("state", "open");
		//Folder attribute: Folder ID value
		JSONObject folderAttrObj = new JSONObject();
		folderAttrObj.put("id","-1");
		folderObject.put("attr", folderAttrObj);
		
		
		//Creating the folder list as Children to the My Folder tree node
		JSONObject childJSONObject = null;
		JSONArray childrenArray = new JSONArray();
		JSONObject childAttrObj = null;
		
		if (null != admFolderDTOList) {
			for (AdmFolderDTO admFolderDTO: admFolderDTOList) {
				childJSONObject = new JSONObject();
				
				//Folder Display Name
				childJSONObject.put("data", admFolderDTO.getFolderName());
				
				//Folder Open/Close Status
				childJSONObject.put("state", "open");
				
				//Folder attribute: Folder ID value
				childAttrObj = new JSONObject();
				childAttrObj.put("id", admFolderDTO.getFolderId());
				childJSONObject.put("attr", childAttrObj);
				childAttrObj = null;
				
				//Add to Children Array
				childrenArray.add(childJSONObject);
				childJSONObject = null;
				}
			folderObject.put("children", childrenArray);
		}
		return folderObject;
	}
	
	/**
	 * Method is called to get All Candidates associated to the Logged-in User	 
	 *  
	 * @param request
	 * @param response
	 * @param session
	...
	public @ResponseBody
	JSONObject getAllCandidates(HttpServletRequest request,
			HttpServletResponse response, HttpSession session,
			ManageJobSeekerForm manageJobSeekerForm,  @RequestParam("resumeId") int resumeId) {
		JSONObject candidatesJSONObject = new JSONObject();
		if (resumeId == 0) {
			List<ManageJobSeekerDTO> manageJobSeekerDTOList = new ArrayList<ManageJobSeekerDTO>();
			try {
				manageJobSeekerDTOList = manageJobSeekerService
					.retrieveAllResume((Integer) session
							.getAttribute(MMJBCommonConstants.USER_ID));
				candidatesJSONObject = convertJobSeekerDtoToJsonObject(manageJobSeekerDTOList);
			} catch (JobBoardServiceException jbex) {
				LOGGER.error("Error occured while Retriving folder json_data", jbex);
			}
		}
		return candidatesJSONObject;
	}
	
	/**
	 * Convert the ManageJobSeekerDTO to the required JSON format to display as Tree Structure
	 * @param admFolderDTOList
	 * @return
	 */
	private JSONObject convertJobSeekerDtoToJsonObject(
			List<ManageJobSeekerDTO> manageJobSeekerDTOList) {

		//Initial Folder with Name "My Folders"
		JSONObject candidatesObject = new JSONObject();
		candidatesObject.put("data", "My Candidates");
		//Folder Open/Close Status
		candidatesObject.put("state", "open");
		//Folder attribute: Folder ID value
		JSONObject candidatesAttrObj = new JSONObject();
		candidatesAttrObj.put("id","-1");
		candidatesObject.put("attr", candidatesAttrObj);
		...
		if (null != manageJobSeekerDTOList) {
			for (ManageJobSeekerDTO manageJobSeekerDTO: manageJobSeekerDTOList) {
				...
				//Folder Display Name
				childJSONObject.put("data", manageJobSeekerDTO.getResumeName());
				
				//Folder Open/Close Status
				childJSONObject.put("state", "closed");
				
				//Folder attribute: Folder ID value
				childAttrObj = new JSONObject();
				childAttrObj.put("id", manageJobSeekerDTO.getResumeId());
				childJSONObject.put("attr", childAttrObj);
				childAttrObj = null;
			...
			candidatesObject.put("children", childrenArray);
		}
