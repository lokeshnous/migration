	...
	public JobSearchResultDTO jobSearch(final String searchName,
		...
		if (serverAccessibility) {
			...
			if ("".equalsIgnoreCase(paramMap.get("titlesearch"))
				...
				response = readSolrServerDetails.getSolrResponse(serverDetailsMap, 
						solrQueryDetails, paramMap, start, rows);
				...
				//For displaying the results. Should be removed when UI will come.
