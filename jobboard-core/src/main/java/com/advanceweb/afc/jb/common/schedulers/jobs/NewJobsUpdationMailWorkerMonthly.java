...
@Service
@Qualifier("NewJobsUpdationMailWorkerMonthly")
public class NewJobsUpdationMailWorkerMonthly implements JobWorker {

	/** The Constant LOGGER. */
	private static final Logger LOGGER = Logger
			.getLogger(NewJobsUpdationMailWorkerMonthly.class);
	...
	public void executeJob() {
		LOGGER.info("New Jobs Updation Mail Worker.-> Execution Of  Monthly Job Scheduler Started.....");
		...
		for (SaveSearchedJobsDTO saveSearchedJobsDTO : savedSearch) {
			...
			if (null != saveSearchedJobsDTO.getEmailFrequency()) {
				emailFrequency = Integer.valueOf(saveSearchedJobsDTO
						.getEmailFrequency());
				if (emailFrequency > 0
						&& emailFrequency == MMJBCommonConstants.MONTHLY_SCHEDULER) {
					emailService.sendMailBySavedSearch(
							MMJBCommonConstants.SCHEDULE_MONTHLY, inputParams,
							saveSearchedJobsDTO, stringNew);
				}
			}

		}
		LOGGER.info("New Jobs Updation Mail Worker.-> Execution Of  Monthly Job Scheduler Executed Successfully.....");
	}
