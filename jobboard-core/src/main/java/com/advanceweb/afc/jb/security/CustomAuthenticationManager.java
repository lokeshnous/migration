...
public class CustomAuthenticationManager implements AuthenticationManager {
	...
	public Authentication authenticate(Authentication auth)throws AuthenticationException {
	 ...
	 public Collection<GrantedAuthority> getAuthorities(int userId) {
			...
			if ( userRole.contains("merion_admin")) {
				authList.add(new GrantedAuthorityImpl("ROLE_MERION_ADMIN"));
			}
			 
			 if ( userRole.contains("jobseeker")) {
					authList.add(new GrantedAuthorityImpl("ROLE_JOB_SEEKER"));
				}
			 
			 if ( userRole.contains("facility_admin")) {
					authList.add(new GrantedAuthorityImpl("FACILITY_ADMIN"));
				}

			 if ( userRole.contains("facility_user")) {
					authList.add(new GrantedAuthorityImpl("ROLE_FACILITY_USER"));
				}
			return authList;
	  }
