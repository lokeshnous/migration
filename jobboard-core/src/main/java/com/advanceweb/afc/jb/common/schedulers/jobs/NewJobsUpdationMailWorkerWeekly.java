...
@Service
@Qualifier("NewJobsUpdationMailWorkerWeekly")
public class NewJobsUpdationMailWorkerWeekly implements JobWorker {

	/** The Constant LOGGER. */
	private static final Logger LOGGER = Logger
			.getLogger(NewJobsUpdationMailWorkerWeekly.class);
	...
	public void executeJob() {
		LOGGER.info("New Jobs Updation Mail Worker.-> Execution Of  Weekly Job Scheduler Started.....");
		...
		for (SaveSearchedJobsDTO saveSearchedJobsDTO : savedSearch) {
			...
			if (null != saveSearchedJobsDTO.getEmailFrequency()) {
				emailFrequency = Integer.valueOf(saveSearchedJobsDTO
						.getEmailFrequency());
				if (emailFrequency > 0
						&& emailFrequency == MMJBCommonConstants.WEEKLY_SCHEDULER) {
					emailService.sendMailBySavedSearch(
							MMJBCommonConstants.SCHEDULE_WEEKLY, inputParams,
							saveSearchedJobsDTO, stringNew);
				}
			}

		}
		LOGGER.info("New Jobs Updation Mail Worker.-> Execution Of  Weekly Job Scheduler Executed Successfully.....");
	}
